...
def calculate_crisis_budget(income):
    return{
        "needs": income * 0.7,
        "wants": income * 0.1,
        "savings": income * 0.2
...
def check_budget_status(needs_total, wants_total, budget, leftover):
    alerts = []

    if needs_total > budget["needs"]:
        alerts.append("Needs exceed recommended budget.")

    if wants_total > budget["wants"]:
        alerts.append("Wants spending too high")
...
def run_budget_analysis(income, income_type, expenses, crisis_mode=False):
    ...
    needs_total, wants_total = categorize_expenses(expenses)
    percentages = calculate_percentages(needs_total, wants_total, monthly_income)
    ...
    alerts = check_budget_status(needs_total, wants_total, budget, leftover)
